...
const StateWiseNewsAggregator = () => {
    ...
    const fetchNewsByCountry = async () => {
        const accessKey = 'a7d27b3d6212b371de158124c9e08d8b'; // Replace with your MediaStack API key
        const countryCode = countries.find(country => country.name === selectedCountry).code;
        const url = `https://api.mediastack.com/v1/news?access_key=${accessKey}${countryCode ? `&countries=${countryCode}` : ''}`;
                    ...
                    {articles.length > 0 ? (
                        articles.map((article, index) => (
                            <div key={index} className="article-card1">
                                {article.image && (
                                    <img src={article.image} alt="Article" className="article-image1" />
                                )}
